...
class PAF(nn.Module):
    """Pixel-wise attention fusion from SCSegamba."""
    ...
    def forward(self, base_feat: torch.Tensor,
                guidance_feat: torch.Tensor) -> torch.Tensor:
        ...
        guidance_query = F.interpolate(
            ...
            mode='bilinear',
            align_corners=False)
        similarity_map = torch.sigmoid(
            self.channel_adapter(base_key * guidance_query))
